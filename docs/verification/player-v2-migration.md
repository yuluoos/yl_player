...
## Apple Hardening Task 1 — managed fallback decomposition

The consolidation BASE `98844babd865d3af7e9fe65ea90e619be9ace88c` was accepted after its scoped, broad and fix reviews. That acceptance supersedes historical pending-review descriptions above without rewriting their checkpoint evidence. Hardening Task 1 subsequently characterized the original backend and extracted demux, video, audio, presentation, recovery and a typed managed session in that order. The backend decreased from 2177 to 88 nonblank lines; the session retains the cross-component seek, track-switch, recovery-install and teardown transactions. Resource/algorithm owners are `YlDemuxPipeline`, `YlVideoPipeline`, `YlAudioPipeline`, `YlPresentationCoordinator` and `YlRecoveryCoordinator`. The existing clock equations, scheduling thresholds, platform compatibility branches and renderer/decoder implementation bytes remain unchanged.

Six new shared characterization methods first ran against the original backend in `5aaa33c`. Existing historical test identities and source pin `ce368c55ad291b163b02157b487dbbf798a69892` remain unchanged; the current fixture inventory is now 259 iOS and 221 macOS methods. The constants verifier uses an explicit seven-file Counter aggregate for each former backend comparison, preserving multiplicity and every original `old_file`, `old_tokens` and `allowed_delta`; no wildcard or scanner waiver was introduced.

Hardening rulings R6–R9 constrain this change. R7 keeps host/open coordinator/slot as the sole public prepare/commit/rollback authority, preserving iOS prepare-before-quiesce and macOS quiesce-before-create. The backend owns one already-prepared session and delegates activation; it does not introduce a competing transaction. R8 permits the existing outer string command adapter only for Task 1: Task 2 must remove command/source/preparation maps across the shared protocol, coordinator, AVPlayer and fallback paths before release. No managed/bounded/hardwareRequired or pluginManaged-audio capability is newly advertised here.

Each extraction received the existing complete iOS and macOS `--unit-only` gates plus the 57-comparison constants gate. Final tested source is `360d9a0b8df30c0634d840315ec44c1eabe97dc6`; the orchestration documentation commit only removes a trailing blank line from tested Swift. A single sixth-extraction compile failure from an unqualified cancellation-token owner was corrected in that source revision before rerunning the gate. R9 records one evidence limitation: Xcode rotated the audio checkpoint's iOS xcresult before its runtime summary was exported. The full gate log, original observed exit 0 and eleven integration passes survive, but its exact XCTest runtime counts are unavailable and must not be inferred from adjacent runs or source declarations. Later full results and final exact case/skip identities were archived before subsequent native runs. This Task 1 implementation remains subject to the controller's independent BASE-to-HEAD review.

Task 1 does not resolve physical-device/hardware coverage, minimum-runtime/endurance evidence, existing Swift 6/build-tooling debt or the later runtime-inventory guard. Portable immutable-artifact verification still validates the accepted lock, signed source and all 18 canonical entries; this decomposition does not alter the artifact, recipe, lock, Pigeon transport or HLS credential/intent context.

### Task 1 review fix round 1

The first Task 1 review required component lifecycle encapsulation and an actual-backend seek-order test. The fix at `4ba46b812f8b6c816f06ed0d5ac5bf85e09e3f41` makes raw decoder/renderer/media/worker/clock/scheduler/display/retry resources private, with typed prepare/install/reset/detach and retired-resource operations. Session retains atomic cross-component generation and main/worker sequencing; its size alone is not ownership evidence. A seventh shared characterization now holds real demux worker work and actual demux seek, checks queue/texture/audio/decoder effects and stale audio/video suppression, and detects both omission and reordering of the real wired audio reset. Per R10, this is current fix evidence: it does not retroactively supply the missing pre-extraction observation.

The completed fix's one full iOS gate passed260 actual cases (257 passed,3 unchanged hardware skips) and11 integrations; macOS `--unit-only` passed222 actual cases with no skips. Exact runtime identities and full result bundles were retained. Original six-case provenance is distinguished from the new seventh case in the manifest. Existing warning debt remains; the new controlled concurrency test additionally has two Swift6 non-Sendable capture warnings and does not establish Swift6 readiness.

R11/R12 authorize only three structural literal folds: audio anchor false8→1, audio generation increment1 four→one, and recovery expectation false2→1. All original57 old-source/token/allowed-delta rows remain unchanged; a separate finite record validates exact owner method bodies and lifecycle invocation mappings before normalizing multiplicity. R13 stores the complete60665-byte FIX_BASE source as one10858-byte deterministic gzip evidence fixture, content-addressed by SHA256 `e5d9244c6ddc0580656ed58d2604b452fda0efe24d152bca082e984c25358413`, outside Swift compilation. Validation checks/caches decompressed content and works in an actual no-git checkout with PATH empty; no new CI fetch or remote reachability dependency is introduced. Nine verifier tests, four provenance tests and57 current constants comparisons pass. These structural records do not substitute for behavior/source review, and all later hardening authority/capability/device obligations remain unchanged.


### Execution cadence update — 2026-09-09

By explicit user instruction, remaining implementation now uses focused checks and defers repeated full gates and independent reviews to one final acceptance stage. [Execution workflow](player-v2-execution-workflow.md) records R15, its scope, evidence requirements and the risk of later cross-module rework. Historical completed checks above remain historical evidence; this cadence change does not claim current Task 2 or final v0.2 acceptance.

### Hardening Task 2 migration boundaries (R14–R16)

Source assessment and Load now share typed native routing; commands and source preparation no longer use string/map dispatch. Production managed networking, bounded buffering and required-hardware guarantees remain staged until Tasks 3–5 prove enforcement. Exact validated network-request settings are retained in `YlAppleNetworkOptions` for Task 3, without the legacy clamps.

R14 closes behavioral migration equality at accepted Task 1 `65cdfd5`: the root consumer-verification fixture retains 57 comparisons, 98 source inputs, three finite folds and R13 evidence (224,982 compressed bytes). R16 separately closes source-copy/declaration origin proof at verified `81d261c`: 86 source inputs plus unchanged metadata (161,198 compressed bytes). The original copy-origin gate does not pass against the later `65cdfd5` live HLS source, and this change does not claim otherwise. Both default foundation invocations now verify pinned historical bytes without live Apple source paths or Git/network access; explicitly named live comparison utilities remain for extraction tests.

These boundaries preserve completed migration provenance while later tasks deliberately change behavior. Their cost is retained compressed source evidence and loss of current-source lexical/hash equality checks; focused contract checks and the final independent review cover current behavior. R15 defers full regression matrices and independent review until all remaining implementation is ready, so Task 2 remains final-acceptance pending after its focused checks.


## Apple hardening durable decisions and evidence (Tasks 3–8)

The following records archive Hardening rulings, whose numbers are separate from the earlier consolidation sequence. All implementation checkpoints remain final-acceptance pending under R15. Historical evidence above is preserved.

### Hardening Tasks3–5 checkpoints

Task3 75ea95cf5b9aaee854f6d821addfee3b2fde1193 enables production managed networking after the user explicitly authorized managedNetwork:true and corresponding expected outcomes. Task4 9e9af31951f232598d0b0d0c4e0538ad35d650d9 enables bounded policies with actual lifetime accounting. Task5 0ed69de6f7c397ee0d8afe7919e9ca1c228108d2 enables hardware evidence after positive exact-resource proof. All are IMPLEMENTED_FOCUSED_CHECKS_PASSED_FINAL_ACCEPTANCE_PENDING under R15; these checkpoints supersede earlier staged-capability descriptions only for implementation, not final acceptance. Source inventory counts are not runtime results.

Hardening R17 introduces managed-only Network.framework/system-TLS HTTP/1.1 framing because actual URLSession experiments withheld final response-header visibility until body delivery. It retains the exact deadline/retry authority and credential lineage, bounded64KiB aggregate headers/16KiB receive, safe framing errors and explicit unsupported proxy/protocol rejection. platformDefault/HLS remain on their compatible paths. Cost: additional bounded framing implementation and supported configurations narrower than the system stack; no HTTP2/3, custom TLS, proxy authentication or silent ATS/proxy bypass. Preserve failed real-header experiments and their passing transport counterparts. Anchored local TLS tests prove isolated verification behavior; production system-root positive success is a separate evidence item.

Hardening R18 uses one Player ledger's opaque-HLS retention lease as mutual admission exclusion between bounded playback and package-controlled HLS. This is an ownership marker, not measured HLS bytes or a second budget. Actual owners, records and every retained send/delegate payload keep it beyond cancel/map clear/session change; only actual final release re-admits the incompatible route. Cost: policy.unsupported during transitions while an incompatible prior owner remains; no automatic retry/downgrade and no promise every replacement fits. Both directions and a held callback across an intervening route were focused tested.

Hardening R19 permits only the new real bounded H264 fixture test's explicit Simulator+actualH264+VTIsHardwareDecodeSupported=false skip with capability attachment. It does not broaden the original three hardware skip identities. Mandatory controlled-VT real HTTP/demux/audio/ledger counterpart passes, but physical-iOS real bounded decoding remains unverified. Cost: Simulator evidence does not prove that hardware-dependent requirement.

Hardening R20 corrects the plan's nonexistent candidate deadline with a private injectable5second monotonic hardware-evidence-stage bound, independent of network/overallLoad and existing2second buffer waits. The exact prepared decoder is positively proven before commit and transferred, not replaced by a second decoder. Deadline/cancel retires publication once while real native owners/permits/tokens survive until return. Preserve inherited macOS single-decoder admission: strict overlap can safely reject before touching accepted playback. Cost: slow positive probes may be rejected, uninterruptible probes retain resources until return, and strict macOS replacement may require stopping/releasing the old decoder. Every recreation must prove its own installed evidence; AVPlayer stays unknown. No candidate frame/audio activation during private preparation.

Task4 ledger metrics are conservative assigned package-payload reservations, not RSS. One ledger includes active/candidate/retired scope tokens and actual compressed/sample/shared-CMBlockBuffer/video/current-picture/PCM/send ownership; media timing ends independently from payload bytes. The corrected CM backing-lifetime association must not regress to opaqueSwift CMSetAttachment, which caused actual macOSVT-19640. Fixed500ms backpressure and2000ms real hardware proof are distinct evidence. Task5 supplies immutable CFBoolean-only installed evidence and shares the existing metrics collector.

### Task3 TLS fixture incident and cleanup

During the first macOS TLS fixture experiment, a test PKCS12 import omitted memory-only behavior and imported two generated leaf identities plus their generated CA into the login keychain. The incident was disclosed to the user. The worker removed exactly those generated identities/certificate by fingerprint (delete-identity for leaves, delete-certificate without -t for CA); no trust-settings change occurred. Controller independently confirmed all three certificates absent; private-key deletion is worker-observed, not a broad independent key enumeration claim. Subsequent imports use memory-only behavior. No global trust installation, permissive production verifier or broad keychain repair was used.

Preserve task-3-tls-cleanup-controller-receipt.json, task-3-tls-fixture-controller-receipt.json, task-3-logs/tls-fixture-import-cleanup.log and the Task3 report until this account is durable. A first10year fixture leaf failed actualSecTrust temporal validity; a disposable90day fixture with the same CA/SAN resolved that fixture issue. Fixed validation date is isolated test-verifier behavior only; production uses current-time/system trust. Anchored TLS positive and untrusted/hostname negative tests passed; do not call this a positive production system-root handshake.

### Evidence pointers and unresolved scope

Task3 controller receipts distinguish unchanged transport proof from actual final capability-activation tests. Task4 controller receipt validates46 current/committed file hashes and final unique macOS46PASS/iOS61PASS+1R19SKIP against supplying runtime case lists and summaries. Task5 controller receipt validates23 current/committed file hashes and final unique macOS49PASS/iOS40PASS+1R19SKIP; earlier failing fixture runs are retained, corrected by exact reruns. Actual macOSVT committed hardware; Simulator strict request rejected honestly. Full matrices and one coordinated independent review remain deferred until all implementation completes. PhysicaliOS, minimumOS, Intel-native decoding, Instruments/memgraph, endurance and remoteCI evidence must remain unverified unless actually obtained. Keep existing warnings and historical evidence gaps with their specific owners; no blanket clean/Swift6ready claim.

### Task6 later checkpoint

HardeningTask6 91aaa3f3d9f5618a02ad8183b267a6eab4a9b7b6 enables existing pluginManagedMediaPlayback policy at typed native boundaries. A shared process driver/lease coordinator keys ownership by registryUUID+stablePlayer; per-Player output permits, host/session identity and existing commit emitter fence late cleanup. AppManaged remains default and makes zero global calls. Pause retains leases, Stop/dispose/currentterminalfailure/detach release relevant owners, finallease deactivates only package-owned activation. Interruption/lifecycle resume honors priorintent and same gate. A failed native deactivation retains evidence of package activation for a later last-release retry rather than claiming success. No externalactive-state inference.

Actual genuine regressions corrected: autoplay activation was initially after destructive fallbackreplacement; preflight now inside existingcommit afterprivateprepare/willCommit, before slotmutation, and rollback restores oldpermit before output. One-shot audioengine start errors could evade terminalcleanup in retainedseek/recovery; `onOutputFailure` now reaches the exact-current-renderer-filtered main/session terminal path. No private hardware preparation audioacquisition, no retiredbackend-deinitrelease.

Task6 controller receipt verifies24current/committed hashes and33actualuniquePASS (15iOS/18macOS), zero skips/failures. Fake-driver realregistry/host tests establish ownershiplogic; actualiOSdriver compiles with playback/moviePlayback/notifyOthers calls. This does not prove physical-device multiFlutterEngine/interruption integration. Last testedtargetsiOS15/macOS12, runtimesiOSSimulator26.5/nativearm64mac26.6.2, inheritedwarningsretained. Fullmatrix/independentreviewpendingR15.


### R21 — default-policy geometry integration versus historical hardware assertion

Ruling: reconcile only the exact iOS default-policy local H264/AAC MKV FirstFrame/geometry case's historical unconditional decoderMode.hardware assertion. Keep systemDefault, require actual successful Load/Ready/FirstFrame and all exact public320x180 encoded/display/PAR1/rotation0/texture/seek assertions. Record actual safe decoderMode and fixed framework identity in the runtime log; preserve identity assertion. Do not replace the hardware assertion with a tautological enumeration assertion, force hardware output, change production evidence mapping, request hardwareRequired just to take an early rejection branch, or silently return before geometry. For this default-policy positive geometry case, any Load failure fails the case. Retain deterministic native hardware/software/unknown mapping, strict exact-resource positive evidence and actual strict-hardware/environment outcome cases as their separate Task5/8 proof. The existing macOS successful hardware assertion/evidence is unchanged unless a distinct actual conflict is established.

Why: Task7 real iOS Dart run reached FirstFrame but reportedunknown at the old hardware assertion before geometry. Parent read defaultcontroller/loadSession case (no decoderPolicy), Task5 immutableCFBoolean evidence, managedstate transfer and collector's direct evidence mapping. Task5 permits systemDefault software/unknown and already recorded actualSimulator strict decoder.unavailable. Decoder frameworkidentity is not positive hardware proof. An unconditional hardware assumption inherited from v0.1 no longer expresses the default-policy contract; preserving it would prevent required positive geometry validation on a successfully decoding supported route. Worker traced evidence and confirmed no Task7 mapping loss.

Cost: this exact default-policy iOS geometry case no longer certifies hardware acceleration; physicaliOS positive hardware remains unverified and cannot be replaced by this Simulator success. Existing independent strict/native evidence requirements are unchanged. Record the original failed attempt, actual mode, final successful geometry result, and this explicit reconciliation in Task7 report, Task8 migration and finalreview. Other legacy helper/error-acceptance paths are not automatically reconciled by this ruling; Task8 evaluates exact supported fixture outcomes and cannot treat absent execution as pass.

### Hardening Task 7 actual geometry, metrics and diagnostics

Task 7 `d058b92de661b0e6140254c3de390302309dbcc2` is implemented with focused checks passed; final acceptance pending. The exact public local MKV cases on both platforms passed Load/READY/FirstFrame, encoded/display 320x180, PAR 1, remaining rotation 0, texture and seek. The iOS original attempt reached FirstFrame but failed the inherited hardware assertion; under Hardening R21 its exact rerun passed with actual decoder mode `unknown` and retained VideoToolbox identity. macOS retained and passed its hardware assertion. This iOS default-policy case proves no hardware acceleration; strict hardware and physical proof remain separate.

The geometry wire contract carries clean-aperture sample dimensions before PAR and unapplied rotation. View applies PAR once, then rotation; already-oriented output bakes effective dimensions and emits PAR 1/rotation 0. The initial Task 7 self-check missed double PAR; the downstream contract check caught it before commit. Final wire cases prove 720x576/PAR16:15/rotation90 -> effective 576x768, plus non-square pixels and already-oriented output. There is no deferred geometry mismatch handed to View.

The existing collector receives typed ready/first-frame/drop/underrun/reconnect/rebuffer signals. Unobserved metrics remain absent, measured zero survives, Load-to-Ready includes private preparation, repeated intervals are deduplicated, and reconnect IDs differ from request retries. Periodic deltas omit static geometry/decoder/track fields. Diagnostic mapping now re-sanitizes every native/Pigeon boundary and HLS listener/502 error body. Task 7 classified all eleven `absoluteString` references as specific private asset/request/cache/credential flows; this is not a blanket filename exception. No raw error description is published.

Final supplying runtime union: 35 unique iOS PASS and 32 unique macOS PASS, zero skips/failures; two actual positive Dart cases (one each). Controller verified 36 current/committed file hashes. Focused source matching diagnostic and PAR fix runs compiled both platforms; unaffected matching cases were reused explicitly. Original macOS command text was not saved before a model-capacity interruption: its actual XCResult, PID, times, cases and Dart log survive, but missing flags must not be reconstructed as historical fact. Retain the Task 7 report, commands, hashes and controller receipt through final acceptance.

Task 1's two additional canonical `YlManagedFallbackCharacterizationTests.swift` non-Sendable SeekControl/backend queue captures remain distinct test-concurrency debt. Existing new host Dispose capture, weak-variable, XCTest floor, GPG trust/build-tooling warnings and historical origin gaps retain their original owners above. Passing Swift 5 mode is neither warning-free nor Swift 6 readiness. Full matrix, coordinated review, physical iOS, minimum OS, native Intel, Instruments/memgraph/endurance, remote CI and release evidence remain pending.

### Hardening Task 8 strict integration and remaining acceptance

Task 8 adds one public Dart suite for both Apple platforms: managed Matroska and FLV must load and render; bounded fixtures must publish nonempty assigned ledger metrics within 16 MiB and 100/2000 ms; unsupported managed/bounded/hardware HLS, MP4, MOV, AVI and MPEG TS/PS routes reject before new upstream requests or replacement. Exact retry attempts, no-retry header timeout, redirect credential stripping and all three real authenticated HLS child-origin paths are tested. There is no policy.unsupported success escape for the supported fixtures. HardwareRequired either produces actual hardware evidence or returns decoder.unavailable with no committed candidate; unknown/software never becomes a committed public snapshot. Native controlled hardware success remains mandatory.

The public replacement test observes the accepted session advancing while a gated candidate prepares, then forces HTTP404 and checks pause/play authority; cancellation separately forbids a candidate firstFrame. The generated Pigeon messenger test holds an actual callback acknowledgement and verifies native FIFO blockage and subsequent public READY/FirstFrame. Public controller tests exercise both Load reply/state orders and retained READY through rebuffer. No handwritten app channel, exported inspection SPI or production test hook was added. Two-player public audio lifecycle and actual native iOS AVAudioSession driver/category/mode tests complement prior injected-driver exact lease evidence; they do not establish a physical multi-FlutterEngine/interruption scenario or infer global active state. macOS retains its explicit no-global-session driver semantics.

Two concrete dependencies were corrected in this task:

- **FLV AAC authority.** Actual RED inspected the repository `h264_aac.flv` as initial bridge SoundRate 44100 with AudioSpecificConfig bytes `11 88 56 e5 00`, which describe 48000 Hz mono. The FLV tag's SoundRate/SoundType fields are placeholders for AAC; the sequence header is authoritative. Initial and reopened FLV inspection now normalize rate/channels from a complete supported AAC-LC config before duration and conversion. Plain LC and its complete inactive SBR sync extension are recognized; unsupported objects/channels/rates, truncation, active SBR/PS, and invalid trailing data remain unrecognized. Bounded duration still rejects a metadata/config mismatch; Matroska is not normalized by this FLV exception. The binary bridge, recipe, retained byte budget and hardware policy were not changed. Cost: the deliberately narrow LC parser does not add HE-AAC or arbitrary extension support. The interpretation follows the [FLV AudioTagHeader specification](https://veovera.org/docs/legacy/video-file-format-v10-0-spec.pdf) and [FFmpeg FLV handling](https://github.com/FFmpeg/FFmpeg/blob/master/libavformat/flvdec.c).
- **Strict initial publication.** The host previously published an empty committed session snapshot with unknown decoder mode before the already-proven backend's hardware snapshot. Only hardwareRequired now defers that empty publication until the actual prepared backend state; it does not derive evidence from the requested policy. All delivered committed-state envelopes, native host state after Load, immediate public controller state after await-Load, READY and firstFrame are asserted. AppleLoadReply carries request/session identity only, and the public session has no initialState field: existing reply/state pairing remains the public barrier. Nonstrict initial publication and accepted-session authority remain unchanged.

The first bounded FLV tests used automatic source intent (never explicit onDemand). A finite network FLV body follows the existing live-EOF reconnect policy: one native run published 106 frames over 24 epochs but its last semantic position stayed 62 ms. A still-open body then published 49 frames in one epoch; the test helper still ignored Task 7 periodic deltas. These were fixture/observer assumptions, separate from the original AAC policy.unsupported failure. The final native observer consumes real delta positions/metrics, and the public FLV server keeps one live body open without repeating timestamps. Actual positive playback/progress and unchanged ledger limits remain required; Matroska independently proves EOF/drain. No VOD contract was reclassified or reconnect behavior changed.

The formerly error-accepting default-policy iOS local/multi-track/network/FLV helpers now require actual success. Default network/FLV cases retain VideoToolbox framework identity and log actual decoder mode, without assuming required hardware or accepting a decoder error. The original exact R21 320x180 geometry assertions remain intact. All integration fixture servers are repository-local loopback origins.

Main example gates now bind the actual RunnerTests PBX Sources phase/file references/group to canonical fixture copies, resolve platform conditional test declarations, and compare actual XCResult case identities and summary outcomes. Duplicate, omitted, extra, failed or unknown-skipped cases fail. The three original iOS hardware skip identities retain separately pinned source conditions and exact messages. R19 is a fourth separately named allowance requiring actual iOS Simulator metadata plus the exported inspected-H264/VT-unavailable attachment; its controlled counterpart remains in the full canonical suite. Consumers share the same guard. Native scripts retain unique result bundles and JSON receipts, and CI uploads evidence even on failure. Raw declaration totals include platform conditionals and never count as runtime proof. Focused receipts explicitly name a planned selection; full scripts cannot silently convert a missing case into a skip.
