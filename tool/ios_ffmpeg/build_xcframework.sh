...
bridge_root="$repo_root/packages/yl_player_ios/ios/native/YlFFmpegBridge"
output="$repo_root/packages/yl_player_ios/ios/yl_player_ios/Frameworks/YlFFmpegBridge.xcframework"
...
build_slice() {
  ...
  if [[ $ffmpeg_arch == x86_64 ]]; then
    # Intel Simulator support must not require a host-installed NASM. Device
    # arm64 slices retain FFmpeg's optimized ARM assembly and NEON paths.
