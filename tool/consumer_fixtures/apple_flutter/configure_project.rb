...
host, platform, manager, fixtures, mode = ARGV
...
if mode == 'link'
  ...
  group.files.select { |file| file.path == 'ConsumerLinkProbe.swift' }.each(&:remove_from_project)
  app.source_build_phase.add_file_reference(group.new_file('ConsumerLinkProbe.swift'))
else
