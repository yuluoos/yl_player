...
enum YlSourceRouter {
  ...
  static func route(_ source: [String: Any?]) -> YlAppleSourceRoute {
    ...
    return route(YlAppleSourceDescriptor(
  ...
  static func route(_ source: YlAppleSourceDescriptor) -> YlAppleSourceRoute {
    ...
    if source.hasHeaders {
      return .reject(
        ...
        message: "Custom \(YlApplePlatform.current.displayName) HTTP headers require a compatible native fallback route."
      )
    }
    ...
    if fallbackHints.contains(source.formatHint)
      ...
      return .reject(
        ...
        message: "This source requires a \(YlApplePlatform.current.displayName) native fallback that is not implemented."
      )
    }
  ...
  private static func validatedUrl(for source: YlAppleSourceDescriptor) -> URL? {
