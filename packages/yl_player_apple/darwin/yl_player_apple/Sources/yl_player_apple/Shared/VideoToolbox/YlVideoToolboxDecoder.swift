...
struct YlVTDecodedImage {
  ...
  init(status: OSStatus, pixelBuffer: CVPixelBuffer?, pts: CMTime, duration: CMTime,
       keyframe: Bool, generation: UInt64, reservation: YlVideoDecodeReservation? = nil,
       ownershipToken: AnyObject?) {
...
enum YlVideoToolboxDecodePolicy {
  static let frameFlags: VTDecodeFrameFlags = YlAppleCompatibility.current.limitsVideoReservations
...
final class YlVideoToolboxDecoder: YlVideoToolboxDecoding {
  ...
  func decode(
    sample: CMSampleBuffer,
    generation: UInt64,
    reservation: YlVideoDecodeReservation? = nil
  ) {
    lock.lock()
...
final class YlHardwareVTSessionFactory: YlVTSessionFactory {
  func makeSession(
    ...
    if YlAppleCompatibility.current.limitsVideoReservations {
      imageAttributes[kCVPixelBufferMetalCompatibilityKey as String] = true
    }
