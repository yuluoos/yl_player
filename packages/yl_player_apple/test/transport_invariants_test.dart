...
void main() {
  test(
    'Apple periodic callback stays typed, lossless, and below 768 bytes',
    () {
      final maximal = AppleStateDeltaMessage(
        sessionId: 'apple-$_maxInt64-s$_maxInt64',
      ...
      const MessageCodec<Object?> codec =
          ApplePlayerFlutterApi.pigeonChannelCodec;
      final bytes = codec.encodeMessage(<Object?>[maximal])!;
      final envelope = codec.decodeMessage(bytes)! as List<Object?>;
      final decoded = envelope.single! as AppleStateDeltaMessage;

      expect(bytes.lengthInBytes, lessThan(768));
      expect(decoded.sessionId, 'apple-$_maxInt64-s$_maxInt64');
      ...
      // ignore: avoid_print
      print('APPLE_MAX_CALLBACK_BYTES=${bytes.lengthInBytes}');
    },
  );

  test('Apple callback codec preserves explicit nullable clears', () {
    final clear = AppleStateDeltaMessage(
      sessionId: 'apple-$_maxInt64-s$_maxInt64',
      previousRevision: _maxInt64 - 1,
      revision: _maxInt64,
      sequence: _maxInt64,
    ...
    const codec = ApplePlayerFlutterApi.pigeonChannelCodec;
    final decoded =
        (codec.decodeMessage(codec.encodeMessage(<Object?>[clear]))!
                    as List<Object?>)
                .single!
            as AppleStateDeltaMessage;
  ...
  test('Apple periodic schema cannot carry static or media payload data', () {
    final schema = File(
      'packages/yl_player_apple/pigeons/yl_player_apple.dart',
    ).readAsStringSync();
    final deltaSchema =
        _classBlock(schema, 'AppleStateDeltaMessage') +
        _classBlock(schema, 'AppleMetricsDeltaMessage');
