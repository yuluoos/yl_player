...
void main() {
  ...
  for (final platform in ApplePlatform.values) {
    test(
      ...
      () async {
        final t = FakeTransport();
        final reply = wireCreate()..platform = platform;
        reply.capabilities
          ..deviceProfile = platform.name
          ..availableEngines = [
            AppleEngine.avPlayer,
            AppleEngine.managedFallback,
          ]
          ..hardwareVideoCodecs = ['video/avc', 'video/hevc'];
        final p = await createFake(t, reply: reply);
        addTearDown(p.dispose);
        expect(p.capabilities.deviceProfile, platform.name);
        expect(p.capabilities.availableEngines, [
          YlPlaybackEngine.avPlayer,
          YlPlaybackEngine.managedFallback,
        ]);
        expect(p.capabilities.hardwareVideoCodecs, ['video/avc', 'video/hevc']);
        expect(p.textureId.value, 42);
        expect(p.state.decoderMode, YlDecoderMode.unknown);
      ...
      () async {
        ...
        final p = await createFake(
          ...
          options: YlPlayerOptions(
            decoderPolicy: strict == 'hardwareDefault'
                ? YlDecoderPolicy.hardwareRequired
                : YlDecoderPolicy.hardwarePreferred,
          ),
        );
        addTearDown(p.dispose);
        final candidate = strict == 'managed'
            ? YlNetworkSource(
                Uri.parse('https://media.test/a'),
                networkPolicy: const YlNetworkPolicy.managed(),
              )
            : source;
        final options = YlLoadOptions(
          decoderPolicyOverride: strict == 'hardwareOverride'
              ? YlDecoderPolicy.hardwareRequired
              : null,
          bufferStrategy: strict == 'bounded'
        ...
        final assessment = await p.assess(candidate, options: options);
        expect(assessment.outcome, YlSourceAssessmentOutcome.incompatible);
        expect(assessment.rejection!.code, YlFailureCodes.policyUnsupported);
        expect(assessment.rejection!.category, YlFailureCategory.unsupported);
        expect(assessment.rejection!.scope, YlFailureScope.command);
        expect(assessment.satisfiedRequirements, isEmpty);
        await expectLater(
  ...
  test(
    'per-load non-strict override of hardwareRequired default reaches native assessment',
    () async {
      final t = FakeTransport();
      final p = await createFake(
        t,
      ...
      addTearDown(p.dispose);
      final result = await p.assess(
        source,
        options: const YlLoadOptions(
          decoderPolicyOverride: YlDecoderPolicy.hardwarePreferred,
        ),
      );
      expect(result.outcome, YlSourceAssessmentOutcome.compatible);
      expect(t.calls, ['attach', 'assess']);
    },
  );
  ...
  test('completion-window deltas use the latest authoritative state', () async {
    ...
    t.loads.single.complete(
      AppleLoadReply(loadRequestId: 'load-1', sessionId: 's1'),
    );
    await loading;
  ...
  for (final asEvent in [false, true]) {
    for (final duringAssessment in [false, true]) {
      test(
        ...
        () async {
          ...
          final assessment = Completer<AppleAssessmentReply>();
          ...
          t.loads.last.complete(
            AppleLoadReply(loadRequestId: 'load-2', sessionId: 's2'),
          );
    ...
    test(
      ...
      () async {
        ...
        addTearDown(p.dispose);
        await commit(t, p);
        t.assessment = () => Completer<AppleAssessmentReply>().future;
        final loading = p.load(source);
        ...
        if (asEvent) {
          t.callbacks!.onPlaybackFailed(
            ApplePlaybackFailedMessage(
              ...
              sequence: 11,
              occurredAtMs: 1,
              failure: wireFailure(scope: AppleFailureScope.player),
            ),
          );
        } else {
          t.callbacks!.onState(
  ...
  test(
    ...
    () async {
      for (final inAssessment in [true, false]) {
        final t = FakeTransport();
        final p = await createFake(t);
        final oldAssessment = Completer<AppleAssessmentReply>();
  ...
  test('mismatched echoed request cannot pair or revive a Load', () async {
    ...
    t.callbacks!.onState(wireState(session: 's1', revision: 1, sequence: 1));
    t.loads.single.complete(
      AppleLoadReply(loadRequestId: 'load-2', sessionId: 's1'),
    );
    await failure;
    expect(p.state.failure!.scope, YlFailureScope.player);
    await p.dispose();
  ...
  test(
    ...
    () async {
      ...
      expect(engine.engine, YlPlaybackEngine.avPlayer);
      await sub.cancel();
      await p.dispose();
    },
  );
  ...
  test(
    ...
    () async {
      ...
      api.onFirstFrame(wireFrame());
      final failure = ApplePlaybackFailedMessage(
        sessionId: 's1',
        revision: 6,
        sequence: 14,
        occurredAtMs: 100,
  ...
  test(
    ...
    () async {
      ...
      t.stopping = () => Future.error(
        PlatformException(
          code: 'native',
          details: wireFailure(scope: AppleFailureScope.command),
        ),
      );
